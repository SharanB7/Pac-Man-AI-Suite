...
def loadModuleFile(moduleName, filePath):
    """Load a module from a file path using importlib.util."""
    spec = importlib.util.spec_from_file_location(moduleName, filePath)
    if spec is None:
        raise ImportError(f"Could not load spec for {moduleName} at {filePath}")
...
def loadModuleString(moduleSource, moduleName):
    """Dynamically load a module from source code as a string."""
...
if __name__ == '__main__':
    ...
    testCaseModuleName = re.match(r'.*?([^/]*)\.py', options.testCaseCode).group(1)
    moduleDict['projectTestClasses'] = loadModuleFile(testCaseModuleName, os.path.join(options.codeRoot, options.testCaseCode))
